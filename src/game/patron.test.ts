...
describe("The Giant", () => {
  ...
  it("tightens a stalled row ONCE, in place, from the third look", () => {
    ...
    expect(order.requirements[0]?.needed).toBe(3); // the row itself tightened
    const again = g.act(ctx(order, [0, 0], { look: 3 }));
    expect(again.utterance).not.toContain("MORE"); // he only tightens once
    expect(order.requirements[0]?.needed).toBe(3);
  });

  it("half the boxes ticked and no crown promised → the cherry demand, once", () => {
    const g = createGiant();
    const order = createOrder(rows(), 5400);
    ...
    expect(added).toEqual({ kind: "count-in-zone", topping: "cherry", zone: "peak", needed: 1 });
    // Never twice — and never when a cherry zone row already exists.
  ...
  it("clock low + a box empty → a reminder that NAMES the row", () => {
    const g = createGiant();
    // A cherry-zone row already exists, so the demand rule stays quiet and
    // urgency gets its turn (rule order is the tree).
    const order = createOrder(
      [...rows(), { kind: "count-in-zone", topping: "cherry", zone: "peak", needed: 1 }],
      5400,
    );
    const act = g.act(ctx(order, [2, 1, 0], { look: 1, secondsLeft: 20 }));
    expect(act.utterance).toContain("DO NOT FORGET");
    expect(act.utterance).toContain("cherry DEAD CENTER");
