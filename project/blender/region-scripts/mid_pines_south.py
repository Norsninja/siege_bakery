# mid_pines_south.py — COMMISSION: pine stand flanking the south hamlet
#
# Per research/18-region-structure-rules.md (THE SCRIPT CONTRACT §8).
# Band: mid_ (lit + fogged — TRUE LOCAL COLOR, no baked haze).
# Zone: x 2..28, y 145..185 (blender), height cap 18 m. Budget 500 tris.
#
# Design (silhouette first, judged from the post eye at (1.5, 11, 1.6)):
#   - 8 chunky storybook conifers, ONE joined mesh. Each tree = a short
#     fat hexagonal trunk frustum (timber #886060) + 2-3 stacked faceted
#     hexagonal cones for the canopy (dark meadow-green family, alternating
#     #305820 / #386028, each tree scaled +/-10% for variation — a tree's
#     tiers share one color so the canopy reads as a single dark mass).
#   - Heights 8-15 m, hand-placed in two loose, non-grid clusters inside
#     the zone (west clump near x 5-14, east clump near x 16-24), radii
#     sized off tree height so bigger trees claim more footprint — some
#     canopies overlap slightly, which is correct for a natural stand.
#   - NO pink accent: this commission explicitly withholds the brand
#     pink so the south hamlet's roofs (just east of this zone) carry
#     the color instead — the exception noted, not an oversight.
...
#
# Deterministic: all variation (position, height, tier count, color
# factor, hex phase) is hand-authored — no randomness used at all.
...
OWNED = ["mid_pines_south"]
NAME = "mid_pines_south"
PREFIX = "mid_"
BUDGET = 500
ZONE_X = (2.0, 28.0)
ZONE_Y = (145.0, 185.0)
ZONE_ZMAX = 18.0
SIDES = 6                          # faceted hex cross-section (trunk + tiers)
...
# Two loose, non-grid clusters; heights 8-15 m; families alternate so the
# stand reads with a little hue depth, not one flat green slab.
TREES = [
    (6.0,  150.0,  9.0, 2, FAM_A, 1.00, 0.00),
    (10.0, 156.0, 12.0, 3, FAM_B, 0.93, 0.35),
    (5.0,  163.0,  8.0, 2, FAM_A, 1.07, 0.70),
    (14.0, 151.0, 14.0, 3, FAM_B, 0.95, 1.05),
    (18.0, 168.0, 11.0, 2, FAM_A, 1.05, 1.40),
    (22.0, 175.0, 15.0, 3, FAM_B, 0.90, 1.75),
    (24.0, 181.0,  9.5, 2, FAM_A, 1.10, 2.10),
    (16.0, 180.0, 13.0, 3, FAM_B, 0.97, 2.45),
]

TIER2_HFRAC = (0.55, 0.45)
TIER2_RFRAC = (1.00, 0.55)
...
for i, (x, y, H, tiers, fam, mul, phase) in enumerate(TREES):
    ...
    # --- trunk: short fat hexagonal frustum, timber
    ...
    # --- canopy: 2-3 stacked faceted cones, dark meadow-green, one family
    #     per tree, embedded slightly into the trunk top to hide the seam
    canopy_h = H - trunk_h
    canopy_r0 = 0.35 * H
    col = hx(fam, mul)
    hfrac, rfrac = (TIER2_HFRAC, TIER2_RFRAC) if tiers == 2 else (TIER3_HFRAC, TIER3_RFRAC)
...
result = {
    ...
    "checks": checks,
    "notes": (
        "Pine stand, 8 trees, %d tris (budget %d), hex-faceted trunks "
        "(timber #886060) + 2-3 stacked hex cone tiers per canopy, dark "
        "meadow-green families #305820/#386028 alternating with a "
        "+/-10%% per-tree multiplier. Heights 8-15 m in two loose "
        "clusters (west x 5-14, east x 16-24); no grid. NO pink accent "
        "by design (see brief) — the adjacent south hamlet's roofs "
        "carry the color instead. Each tree grounded by its own raycast, "
        "cached before any geometry existed so no self-hits; base sunk "
        "0.3 m each. Deterministic: all variation hand-authored, no "
        "randomness used."
        % (tri_count, BUDGET)),
}
