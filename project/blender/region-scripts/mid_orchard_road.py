"""
COMMISSION: mid_orchard_road
BAND: mid_
ZONE: x 125..165, y 68..79 (blender), height cap 14 m
BUDGET: 500 tris total
BRIEF: A roadside orchard strip between the giants' road (south, y
26..60) and the road hamlet (north, y 81..130) -- fruit trees line the
last stretch of a giant's walk to the table. 6 chunky lollipop trees
(fat tapered trunk + big faceted round canopy) as ONE mesh, staggered
along the narrow east-west strip (never a straight fence -- y zigzags
within the strip, x spacing varies). Mostly meadow-green family, 2 in
blossom pink -- fewer pink than the west orchard since the hamlet's
own pink roofs sit right behind this strip. Each tree grounded on its
own raycast into the rolling meadow.
...
from_pydata face lists to get backwards. Pattern follows the accepted
mid_orchard_west.py precedent verbatim.
"""
...
# Hand-placed, deterministic. Staggered along the narrow east-west strip:
# x spacing varies (not evenly spaced), y zigzags within the strip so
# canopies never line up in a fence. Heights 6.0-10.5 m (per brief 6-11).
# 2 of 6 trees are in blossom (brief wants fewer pink than the west
# orchard's three -- this strip supports the hamlet's own pink roofs).
TREES = [
    dict(x=128.5, y=72.0, h=7.0,  blossom=False, hex="#386028", shade=1.00),
    dict(x=136.0, y=75.8, h=9.0,  blossom=False, hex="#386830", shade=0.95),
    dict(x=143.0, y=70.0, h=6.0,  blossom=True,  hex="#B87890", shade=1.00),
    dict(x=150.5, y=75.5, h=10.5, blossom=False, hex="#386028", shade=1.05),
    dict(x=157.0, y=71.0, h=8.0,  blossom=True,  hex="#A88098", shade=1.00),
    dict(x=162.0, y=74.5, h=6.5,  blossom=False, hex="#386830", shade=1.03),
]
...
result = {
    ...
    "notes": (
        ...
        f"authored heights {h_min:.1f}-{h_max:.1f} m, staggered along the "
        f"road strip (y zigzags 70.0-75.8), "
